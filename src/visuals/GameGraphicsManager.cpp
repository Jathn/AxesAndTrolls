...
GameGraphicsManager::GameGraphicsManager(const std::shared_ptr<GameStateManager>& game_state_manager) 
: map_view_(game_state_manager->getMap(), std::make_pair(100, 100), std::make_pair(1000, 650), game_state_manager->getPlayers()) {
    for (int i = 0; i < 4; i++) {
